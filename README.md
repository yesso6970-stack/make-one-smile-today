...
바이브 코딩 챌린지를 위해 제작한 모바일 퍼스트 PWA입니다. STEP 1에서는 외부 서비스 없이 더미 데이터만으로 완성된 UI 경험을 제공합니다.
...
상단의 전체 미소 수는 실제 전체 사용자 통계가 아닌 데모 시작값에 이 브라우저에서 `상대가 웃었어요`를 누른 횟수를 더한 값입니다. 복사와 앱 공유는 미소 성공으로 간주하지 않아 카운트에 포함하지 않습니다. 실제 전체 사용자 통계는 추후 서버 데이터베이스 연결이 필요합니다.

## Vercel 배포

GitHub 저장소를 Vercel에 연결하면 Next.js 프로젝트로 자동 인식됩니다. 앱 공유 주소를 명시하려면 Vercel 환경 변수에 `NEXT_PUBLIC_APP_URL=https://배포주소`를 설정하세요. 설정하지 않아도 브라우저 공유 기능은 현재 배포 주소를 사용합니다.
